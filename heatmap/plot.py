...
#Last second to show
ends =  [13,23,len(cpus[0])]

for i_show in range(3):
    plt.close()
    ...
    im = plt.imshow(cpus[:,2:ends[i_show]], vmin=0,vmax=100)
...
#cbar.ax.set_ylim(0,100)
    ...
    plt.savefig('heatmap_%d.' % i_show + ext)
    ...
    if dual:
        plt.savefig('rttbw_%d.' % i_show + ext)
    else:
        plt.savefig('rtt_%d.' % i_show + ext)
