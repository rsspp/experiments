...
shift = [0,0,0,0,0,0]
shift[0] = -min([min(d[:,0]) for d in data_a_s]) -  0.9
shift[1] = -min([min(d[:,0]) for d in data_a_s]) + 0.1
shift[2] = -min([min(d[:,0]) for d in data_b_s])
shift[3] = -min([min(d[:,0]) for d in data_b_s])
shift[4] = -min([min(d[:,0]) for d in data_c_s])

fig, ax1 = plt.subplots()
fig.set_size_inches(6,3)

for i,data in enumerate(data_a_s):
    ax1.plot(data[:,0] + shift[i], [np.mean(l) for l in data[:,1]], label=labels[i], color = graphcolor[i], linestyle=linestyles[i % len(series)], marker='None')
...
for i,data in enumerate(data_b_s):
    ax2.plot(data[:,0] + shift[i + 2] , [np.mean(l) for l in data[:,1]], label=labels[i], color = graphcolor[i + 2], linestyle=linestyles[i % len(series)], marker='None') #markers[ int(i / 2)]
...
df = df.sort_values(by=[df.columns[2]]).values

#Background
t = np.asarray(df[:,2])
w = np.asarray(np.ediff1d([0] + t))
t=t[:-1]
b = ax2.bar((t + w / 2) + shift[4], height=h, width=w, color=[lighter(colors[2],.12,1),lighter(colors[2],.05,1)], zorder=-99999)
